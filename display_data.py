...
def display_root_data():
    # Load data.csv from root directory
    df = pd.read_csv(os.path.join('data.csv'), index_col=None)

    fig, ax = plt.subplots()

    ax.plot(df['Time'], df['Susceptible'], color='blue')
    ax.plot(df['Time'], df['Infected'], color='red')
    ax.plot(df['Time'], df['Removed'], color='green')
